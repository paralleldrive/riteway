...
CRITICAL: Return ONLY the TAP YAML block. Start with --- on its own line,
  ...
  test('omits context section when promptUnderTest is empty', () => {
    const userPrompt = 'What is 2 + 2?';
    const result = 'The answer is 4.';
    const requirement = 'Given simple addition, should return 4';
...
CRITICAL: Return ONLY the TAP YAML block. Start with --- on its own line,
    ...
    assert({
      given: 'empty promptUnderTest',
      should: 'omit CONTEXT section from judge prompt',
      actual: judgePrompt,
      expected
    });
  });
  ...
  test('extracts and returns validated test structure from agent output', async () => {
    ...
    try {
      ...
      assert({
        ...
        should: 'return extracted structure with resolved prompt under test',
        actual: result,
        expected: {
          userPrompt: 'What is 2+2?',
          promptUnderTest: 'You are a math helper.',
          assertions: [{ id: 1, requirement: 'Given addition, should add correctly' }]
        }
      });
    } finally {
